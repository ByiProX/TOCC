...
from configs.config import SUCCESS, main_api_v2,rds, db, DEFAULT_PAGE, DEFAULT_PAGE_SIZE, ERR_INVALID_PARAMS, ERR_WRONG_ITEM, \
...
def getGrouplist(client_id):
    ret = {}
    if client_id :
        groupList = BaseModel.fetch_all('client_group_r', ['group_name','group_id','status'] ,BaseModel.where("=", "client_id", client_id))
...
def getClientQunWithGroup(client_id,group_id,page=1,pagesize=30,order=''):
    ...
    if group_id :
        ...
        qunList = BaseModel.fetch_all('client_qun_r', ['chatroomname','group_id','group_info','status'],_where,page=1,pagesize=pagesize,orderBy=order)
...
def chatroom_statistics_chatroom():
    ...
    if(date_type==1):
        ...
        _where = ["and",["<=","run_hour",run_hour],["=","client_id",user_info.client_id],["in","chatroomname",chatroomname_list]] 
    elif(date_type==2):
        timestamp_diff = getTimeStamp(1)
        last_update_time =  timestamp_diff + 600 
       # _where ={"date":timestamp_diff,"client_id":user_info.client_id}
        _where = ["and",["=","date",timestamp_diff],["=","client_id",user_info.client_id],["in","chatroomname",chatroomname_list]]  
...
def getQunInfo(chatroomnames):
    ret = {}
    if chatroomnames :
        qunInfo = BaseModel.fetch_all('a_chatroom', ['chatroomname','nickname','avatar_url'] ,BaseModel.where("in", "chatroomname", chatroomnames))
        if(qunInfo):
            for qf in qunInfo:
                _qfjson = qf.to_json()
