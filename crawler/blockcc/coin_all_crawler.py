        ...
        if priority > 0:
            ...
            for ii in i.children:
                ...
                if xx == 4:
                    ...
                    id = re.search(r'\d+', ii.div["class"][0]).group(0)
                    print 'coin_icon:', 'https://s2.coinmarketcap.com/static/img/coins/16x16/' + id + '.png'
                    coin['coin_icon'] = 'https://s2.coinmarketcap.com/static/img/coins/16x16/' + id + '.png'
                if xx == 6:
...
def update_coin_all():
    ...
    for coin_json in coin_list:
        ...
        notice = allcoinsdefaultsettinginfo(priority, symbol, coin_name, coin_icon, change1d, change1h,change7d,
                                            marketcap, available_supply, price,volume_ex, mineable)
        new_coin_dict[priority] = notice
    new_coin_symbol_set = set(new_coin_dict.keys())
    for new_coin_symbol in new_coin_symbol_set:
        new_coin = new_coin_dict[new_coin_symbol]
        db.session.add(new_coin)
    # 去重插新
    # old_coin_list = db.session.query(allcoinsdefaultsettinginfo).all()
