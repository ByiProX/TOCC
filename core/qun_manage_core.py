...
def set_default_group(user_info):
    ...
    # 如果已经有了默认群，则不去建立默认群
    if cur:
        logger.warning("已有默认分组. user_id: %s." % user_info.user_id)
        return WARN_HAS_DEFAULT_QUN
    else:
        _create_new_group(user_info.user_id, "未分组", is_default_group=True)
        logger.debug("已建立默认分组. user_id: %s." % user_info.user_id)
        return SUCCESS
...
def create_new_group(group_name, token=None, user_id=None, user_info=None):
    ...
    logger.info("添加分组. user_id: %s. group_id: %s." % (user_id, group_info.group_id))
...
def get_group_list(user_info):
    ...
    if not group_list:
        logger.error("无默认分组. user_id: %s." % user_info.user_id)
    ...
    logger.info("获取分组列表. user_id: %s." % user_info.user_id)
    return SUCCESS, res
...
def rename_a_group(group_rename, group_id, user_id):
    ...
    if not group_info:
        logger.error("无法找到该分组. group_id: %s." % group_id)
        return ERR_WRONG_ITEM
    if group_info.user_id != user_id:
        logger.error("用户与分组属于不同用户. group_id: %s. user_id: %s." % (group_id, user_id))
        return ERR_WRONG_USER_ITEM
    if group_info.is_default is True:
        logger.error("默认分组无法更名. group_id: %s. user_id: %s." % (group_id, user_id))
        return ERR_RENAME_OR_DELETE_DEFAULT_GROUP
    group_info.group_nickname = group_rename
    db.session.commit()
    logger.info("重命名成功. group_id: %s." % group_id)
    return SUCCESS
...
def delete_a_group(group_id, user_id):
    ...
    if not user_default_group_info:
        logger.error("无默认分组. user_id: %s." % user_id)
        return ERR_WRONG_ITEM
    ...
    logger.info("已删除分组. old_group_id: %s. user_id: %s." % (group_id, user_id))
    return SUCCESS
...
def transfor_qun_into_a_group(group_id, uqun_id, user_id):
    ...
    if qun_info.user_id != user_id:
        logger.error("用户与群属于不同用户. uqun_id: %s. user_id: %s." % (uqun_id, user_id))
        return ERR_WRONG_USER_ITEM
    ...
    logger.info("转移分组成功. new_group_id: %s. uqun_id: %s." % (group_id, uqun_id))
    return SUCCESS
...
def check_whether_message_is_add_qun(message_analysis):
    ...
    if msg_type == MSG_TYPE_SYS and content.find(u'邀请你') != -1:
        ...
        logger.info("发现加群. user_nickname: %s. chatroomname: %s." % (user_nickname, message_analysis.talker))
...
def _bind_qun_success(chatroomname, user_nickname, bot_username):
    ...
    if len(user_info_list) > 1:
        logger.error("根据nickname无法确定其身份. user_nickname: %s." % user_nickname)
        return ERR_HAVE_SAME_PEOPLE
    elif len(user_info_list) == 0:
        logger.error("配对user信息出错. bot_username: %s. user_nickname: %s" % user_nickname)
        return ERR_WRONG_ITEM
    ...
    if not bot_info:
        logger.error("bot信息出错. bot_username: %s" % bot_username)
        return ERR_WRONG_ITEM
    ...
    if len(group_info_list) > 1:
        logger.error("发现多个默认分组. user_nickname: %s." % user_nickname)
        return ERR_WRONG_ITEM
    elif len(group_info_list) == 0:
        logger.error("无默认分组. user_nickname: %s." % user_nickname)
        return ERR_WRONG_ITEM
    else:
    ...
    db.session.commit()
    logger.debug("user与群关系已绑定. uqun_id: %s." % uqr_info.uqun_id)
    ...
    if len(ubr_info_list) > 1:
        logger.error("找到多个bot关系. user_id: %s." % user_id)
        return ERR_WRONG_ITEM
    elif len(ubr_info_list) == 0:
        logger.error("没有找到bot关系. user_id: %s." % user_id)
        return ERR_WRONG_ITEM
    else:
    ...
    db.session.commit()
    logger.info("绑定群的四个关系. uqbr_id: %s." % uqbr_info.rid)
    return SUCCESS
