...
def add_a_pre_relate_user_bot_info(user_info, chatbot_default_nickname):
    ...
    if len(ubr_info_list) > 1:
        ...
        return SUCCESS, ubr_info_list[0]
...
def _get_a_balanced_bot(user_info):
    ...
    #     alive_bot_username_list.remove(u"wxid_77txprhfmu5n22")

    bot_info_list = BaseModel.fetch_all(BotInfo, "*", where_clause = BaseModel.where("in", "username", alive_bot_username_list))
    bot_info = None
    print bot_info_list
    if bot_info_list:
        bot_info = random.choice(bot_info_list)
