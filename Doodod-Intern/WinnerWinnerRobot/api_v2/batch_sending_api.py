...
from configs.config import SUCCESS, ERR_PARAM_SET, main_api_v2, SHORT_DURATION_TIME_ERROR
from core_v2.batch_sending_core import get_batch_sending_task, get_task_detail, create_a_sending_task, \
    delete_batch_sending_task, create_a_timed_sending_task, recreate_a_timed_sending_task
...
def app_create_a_sending_task():
    ...
    send_time = request.json.get('send_time', 0)
    batch_send_task_id = request.json.get('batch_send_task_id', 0)
    ...
    if not send_time:
        ...
        if not batch_send_task_id:
            ...
            status = recreate_a_timed_sending_task(user_info, chatroom_list, message_list, send_time, batch_send_task_id)
...
@main_api_v2.route("/rewrite_send_time_real", methods=['POST'])
def app_rewrite_send_time_real():
    batch_send_tasks = BaseModel.fetch_all("batch_send_task", "*")
    for batch_send_task in batch_send_tasks:
        if not batch_send_task.send_time:
            batch_send_task.send_time = batch_send_task.create_time
        else:
            if not batch_send_task.send_time_real:
                batch_send_task.send_time_real = batch_send_task.send_time

    for batch_send_task in batch_send_tasks:
        if batch_send_task.send_time_real < int(time.time()):
            batch_send_task.status = 3
