...
from configs.config import SUCCESS, Keywords, Coin, UserSwitch
...
def get_gm_rule_dict():
    ...
    clients = BaseModel.fetch_all(UserSwitch, "*", where_clause = BaseModel.where_dict({"func_auto_reply": 0}))
    client_ids = [r.client_id for r in clients]
    keywords_list = BaseModel.fetch_all(Keywords, "*", where_clause = BaseModel.and_(["not in", "client_id", client_ids], ["=", "welcome", 0], ["=", "status", 1]))
